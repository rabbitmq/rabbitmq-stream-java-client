...
public class ByteCapacity {
    ...
    private static final Pattern PATTERN = Pattern.compile("^(?<size>\\d*)((?<unit>kb|mb|gb|tb))?$", Pattern.CASE_INSENSITIVE);
    ...
    private static final Map<String, Function<Long, ByteCapacity>> CONSTRUCTORS = Collections.unmodifiableMap(new HashMap<String, Function<Long, ByteCapacity>>() {{
    ...
    public static ByteCapacity from(String value) {
        ...
        if (matcher.matches()) {
            ...
            if (unit == null) {
                ...
                return CONSTRUCTORS.getOrDefault(unit.toLowerCase(),
                        s -> {
