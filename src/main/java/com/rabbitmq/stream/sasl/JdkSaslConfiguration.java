 ...
 * <p>
 * This is not the default because SASL is not available on all environments, e.g. Android.
 * This code is based on the implementation from RabbitMQ Java client.
 */
public class JdkSaslConfiguration implements SaslConfiguration {
            ...
            if (serverMechanisms.contains(mechanism)) {
                try {
                    SaslClient saslClient = Sasl.createSaslClient(new String[]{mechanism},
                            null, "RABBITMQ-STREAM", serverNameSupplier.get(), null, callbackHandler);
    ...
    private class UsernamePasswordCallbackHandler implements CallbackHandler {
        ...
        public UsernamePasswordCallbackHandler(CredentialsProvider credentialsProvider) {
            if (credentialsProvider == null || !(credentialsProvider instanceof UsernamePasswordCredentialsProvider)) {
