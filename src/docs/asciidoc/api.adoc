...
* the consumer must periodically *commit the offset* it has reached so far. This is not done
automatically, it must be done in the application code.

The following snippet is an example of offset tracking:

[[tracking-consumer-offset-snippet]]
.Tracking consumer offset
[source,java,indent=0]
--------
include::{test-examples}/ConsumerUsage.java[tag=offset-tracking]
--------
<1> Set the commit interval to 100
<2> Set the consumer name (mandatory for offset tracking)
<3> Count messages
<4> Commit at the specified offset

[NOTE]
.When to commit offsets?
====
Avoid committing the offset for each message. Even though offset tracking is a small and
fast operation, it will make the stream grow unnecessarily, as the broker persists offset
tracking entries in the stream itself.

A good rule of thumb is to commit every few hundreds
...
last duplicate messages.
====

[NOTE]
.Is the offset a reliable absolute value?
====
Message offsets may not be contiguous. This implies that the message at offset 500 in a stream may
not be the 501 message in the stream (offsets start at 0). There can be different types of entries
in a stream storage, a message is just one of them. Committing an offset creates an offset tracking
entry, which has its own offset.

In the snippet above, we use a separate counter to count messages and make sure to commit every 100
messages. We could have avoided this counter and do the modulo operation on the offset directly,
but we would have had no guarantees that we committed every 100 messages (in practice this should
not make much difference in most cases).
====